...
const readlines = () => {
  ...
  // efficient refactor of code uses variable below
  let invalid_chars = [" ", ".", "-", "'", "’", "(", ")"];
  ...
  for (let i = 0; i < countriesArr.length; i++) {
    ...
    for (let j = 0; j < countryName.length; j++) {
      ...
      if (vowels.includes(letter)) {
        vowel_count++;
      } else if (invalid_chars.includes(letter)) {
      ...
      // FIRST REFACTOR OPTION; MORE EFFICIENT STILL VERBOSE
      // if (
      //   letter === " " ||
      //   letter === "." ||
      //   letter === "-" ||
      ...
      // BASIC OPTION; VERBOSE AND DUMMIFIED
...
// NEXT SEE WHAT HAPPENS IN RESULTS WITH "Y" AND TRY TO REMOVE MORE INVALID CHARACTERS BEING READ AS LETTERS (DOT(.); DASH(-); APOSTROPHE(')) ... done Done DONE in most basic aka dumb way (after attempted multiple or/pipe operators in initial)
