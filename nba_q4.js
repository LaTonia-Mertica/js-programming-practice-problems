...
for (const [key, value] of Object.entries(mvpDict)) {
  ...
  if (value === 1) {
    continue;
    // when value equals two add player to specific array
  } else if (value === 2) {
    twoMvpPlayers.push(key);
    // when value equals three add player to specific array
  } else if (value === 3) {
    threeMvpPlayers.push(key);
    // otherwise add remaining player(s) to specific array
  } else {
